...
int dfs(vector<vector<char>>& mapa, int x, int y, char elem)
{
	...
	if (x == 0 && y == 0 )
	{
		if (mapa.size() != 1 && mapa[x + 1][y] == elem) hoyo += dfs(mapa, x + 1, y, elem);
		if (mapa[0].size() != 1&&mapa[x][y + 1] == elem) hoyo += dfs(mapa, x, y + 1, elem);
	}
	else if (x == 0)
	{
		if (y != mapa[0].size() - 1 && mapa[x][y + 1] == elem) hoyo += dfs(mapa, x, y + 1, elem);
		if (mapa.size() != 1&& mapa[x + 1][y] == elem) hoyo += dfs(mapa, x + 1, y, elem);
		if (mapa[x][y - 1] == elem) hoyo += dfs(mapa, x, y - 1, elem);
	}
	else if (y == 0)
	{
		if (x != mapa.size() - 1 && mapa[x + 1][y] == elem) hoyo += dfs(mapa, x + 1, y, elem);
		if (mapa[x - 1][y] == elem) hoyo += dfs(mapa, x - 1, y, elem);
		if (mapa[0].size() != 1&&mapa[x][y + 1] == elem) hoyo += dfs(mapa, x, y + 1, elem);	
	}
	else if (x == mapa.size() - 1 && y == mapa[0].size() - 1)
	{
		if (y != 0&&mapa[x][y - 1] == elem) hoyo += dfs(mapa, x, y - 1, elem);
		if (x != 0&&mapa[x - 1][y] == elem) hoyo += dfs(mapa, x - 1, y, elem);
	}
